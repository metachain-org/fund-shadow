...
export default function Dashboard() {
  ...
  const { makeDonation, isPending, error } = useMakeDonation()

  const handleVote = async (projectId: string, vote: 'yes' | 'no' | 'abstain') => {
    if (!address) {
      toast.error('Please connect your wallet first')
      return
    }

    setIsVoting(prev => ({ ...prev, [projectId]: true }))
    
    try {
      // Convert vote to encrypted amount (simplified for demo)
      // In real implementation, this would use FHE encryption
      const voteAmount = vote === 'yes' ? 1 : vote === 'no' ? 0 : 0.5
      
      // Create encrypted vote data (this would be properly encrypted in production)
      ...
      // Simulate FHE encryption proof (in real app, this would be generated by FHE library)
      const voteProof = btoa(`proof_${projectId}_${vote}_${Date.now()}`)
      
      // Call smart contract with encrypted vote
      await makeDonation(
        parseInt(projectId),
        encryptedVote,
        voteProof,
        `Vote: ${vote} on project ${projectId}`,
        0n // No ETH value for voting
      )
      ...
      setIsVoting(prev => ({ ...prev, [projectId]: false }))
    }
  }
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {mockProjects.map((project) => (
              <ProjectCard
                ...
                isVoting={isVoting[project.id] || false}
              />
            ))}
          </div>
        </div>
